...
def build_schema_info(db_connection):
    ...
    ret = []
    with djc.cursor() as cursor:
        tables_to_introspect = djc.introspection.table_names(
        ...
        for table_name in tables_to_introspect:
            if not _include_table(table_name):
                continue
            td = []
            table_description = djc.introspection.get_table_description(
                cursor, table_name
            )
            for row in table_description:
                column_name = row[0]
                try:
                    field_type = djc.introspection.get_field_type(
                        row[1], row
                    )
                except KeyError:
                    field_type = "Unknown"
